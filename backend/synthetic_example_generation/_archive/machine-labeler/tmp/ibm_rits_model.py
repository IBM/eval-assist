...
from machine_labeler import BaseModel
from machine_labeler.utils.data_utils import (get_unlabeled_ids,
                                              get_examples_pool,
                                              save_results,
                                              save_ids)
from machine_labeler.utils.labeling_utils import (extract_examples,
                                                  get_criteria_block,
                                                  format_query,
                                                  format_prompt)


class IBMRITSModel(BaseModel):
    def __init__(self,
                 model_id,
                 output_format,
                 config):
    ...
    def _call_model(self, prompts, rubrics, output_format, max_attempts=10):
        ...
        # Define generation parameters (assuming these were previously in self.parameters)
        generation_params = {
            'max_new_tokens': self.parameters.get('max_new_tokens', 1024),
        ...
        while prompt_ids and num_batch_attempts < max_attempts:
            ...
            # Create args list for the current batch of prompts
            args_list = [(prompts[i], generation_params) for i in prompt_ids]

            # Process batch using thread_map
            generation_outputs = thread_map(
                self._get_rits_completion,
                args_list,
                max_workers=self.num_parallel_requests,
                desc=f"batch {num_batch_attempts + 1}"
            )
            generation_outputs = list(generation_outputs)

            # Update outputs dictionary with new results
            for idx, output in enumerate(generation_outputs):
                outputs[prompt_ids[idx]] = output

            # Parse outputs and get failed prompt_ids
