...
def get_default_model():
    return db.llmmodel.find_unique(where={"id": "meta-llama/llama-2-70b-chat"})
    ...
    if should_sync:
        ...
        for model in models:
            db.llmmodel.upsert(
                where={"id": model.id},
                data={
                    "create": {"name": model.name, "id": model.id},
                    "update": {},
                },
            )

        # remove models that are no longer available
        db.llmmodel.delete_many(where={"id": {"notIn": [m.id for m in models]}})
...
def run_model(prompt: Prompt):
    ...
    try:
        ...
        responses = list(
            client.text.generation.create(model_id=prompt.model.id, inputs=[prompt.text], parameters=params)
        )
        end = time.time()
        ...
        db.prompt.update(
            ...
            data={
                ...
                "status": ModelRunStatus.FINISHED,
