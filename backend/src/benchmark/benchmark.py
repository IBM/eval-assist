            ...
            if file.endswith(".json"):
                ...
                with open(file_path, 'r') as f:
                    try:
                        ...
                        if ((to_merge_benchmark['name'], to_merge_benchmark['type']) in merged_benchmarks_dict):
                            # it exists, add the evaluator benchmarks to the appropriate 
                            # criteriaBenchmarks.evaluatorBenchmarks list
                            merged_benchmark = merged_benchmarks_dict[(to_merge_benchmark['name'], to_merge_benchmark['type'])]
                            for criteriaBenchmark in merged_benchmark['criteriaBenchmarks']:
                                criteriaName = criteriaBenchmark['name']
                                to_merge_criteria_benchmark = [criteriaBenchmark 
                                                               for criteriaBenchmark 
                                                               in to_merge_benchmark['criteriaBenchmarks'] 
                                                               if criteriaBenchmark['name'] == criteriaName][0]
                                criteriaBenchmark['evaluatorBenchmarks'].extend(to_merge_criteria_benchmark['evaluatorBenchmarks'])
                        else:
                            merged_benchmarks_dict[(to_merge_benchmark['name'], to_merge_benchmark['type'])] = to_merge_benchmark
