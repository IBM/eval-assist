...
class EvaluationNotebookGenerator(ABC):
    ...
    def _add_title(self):
        ...
        title += (f"This notebook was generated automatically from your EvalAssist test case \"{self.params.test_case_name}\". "
    ...
    def _add_evaluation(self):
        self.cells.append(Cell(type="md", content="### Evaluate the responses and print the results"))
...
"""
    ...
    def get_load_criteria_code(self):
        option_map_string = {option["name"]: float("nan") for option in self.params.criteria["options"]}
        return f"""
...
"""
    ...
    evaluator_name=EvaluatorNameEnum.GRANITE3_1_2B.name,
    inference_engine=LiteLLMInferenceEngine(**{json.dumps(self.inference_engine_params, indent=4)}),
    criteria=criteria,
...
"""
    ...
    def get_load_dataset_code(self):
        system_predictions = [{f"system_{i+1}": pred for i, pred in enumerate(instance_preds)} for instance_preds in self.params.predictions]
        return f"""context_variables = {json.dumps(self.params.context_variables, indent=4)}
